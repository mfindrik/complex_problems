...
for i in LOCATIONS:prob += lpSum(assign_vars[(i, j)]*REQUIRE[j] for j in PRODUCTS) \
...
for i in LOCATIONS:
    if use_vars[i].varValue > TOL:
        print("Location ", i, " produces ", \
              [ j for j in PRODUCTS if assign_vars[(i, j)].varValue > TOL ])
